...
```
src/
├── contracts/                # SHARED INTERFACES — implemented first, imported by all
│   ├── types.ts              # All shared TypeScript interfaces and types
│   ├── events.ts             # Event bus types for cross-module communication
...
This section defines how an agent team can implement Phase 1 concurrently. The key insight: **most modules are pure TypeScript with no React or DOM dependencies**, meaning they can be built, tested, and validated in isolation before integration.

### 13.1 Shared Interface Contracts (Must Complete First)

Before any workstream begins, a **Coordinator agent** scaffolds the project and defines the shared contracts in `src/contracts/types.ts`. All agents code against these interfaces. This takes ~1 hour and unblocks everything else.

```typescript
// src/contracts/types.ts — the contract all agents implement against
...
### 13.2 Workstream Definitions

Each workstream is independently implementable. An agent picks up a workstream, reads the relevant spec sections, implements against the shared contracts, writes BDD scenarios and tests, and opens a PR. No workstream requires another workstream's implementation to compile or test — they all depend only on `src/contracts/`.
...
```
    ...
    │Phonics│ │SpaceR│ │ Data │ │Audio │ │ OCR  │ │Theme │ │A11y  │
    │Engine│ │+WordS│ │Layer │ │System│ │Pipe  │ │+Rwrd │ │System│
    └──┬───┘ └──┬───┘ └──┬───┘ └──┬───┘ └──┬───┘ └──┬───┘ └──┬───┘
       │        │        │        │        │        │        │
       └────────┴────────┴───┬────┴────────┴────────┴────────┘
                             ▼
...
#### WS-0: Foundation (Coordinator — must complete first)

**Time:** ~1-2 hours
**Owner:** Coordinator agent
**Spec sections:** §4.1, §12.1, §10.7
**Deliverables:**
- Vite + React + TypeScript project scaffold
- Tailwind CSS configured
- Vitest + vitest-cucumber + Playwright configured
- Dexie.js installed, schema defined (`src/data/db.ts`)
- All shared types in `src/contracts/types.ts` (see §13.1 above)
- Event bus implementation in `src/contracts/events.ts`
- Test fixtures in `tests/fixtures/` (sample profiles, word lists, session histories)
- CI pipeline (`.github/workflows/test.yml`)
- ESLint + Prettier config
- `README.md` with setup instructions

**Exit criteria:** `npm install && npm run typecheck && npm test` passes. All agents can clone and start working.
...
**Time:** ~4-6 hours
**Dependencies:** WS-0 only
**Spec sections:** §6 (Phonics Engine Deep Dive), §5.1.4 (scaffolding behavior)
**Inputs:** `Word.text` (a string)
**Outputs:** `PhonicsResult` (syllables, phonemes, patterns, hints, related words)
**Files to create:**
- `src/core/phonics/engine.ts` — main `analyzeWord()` function
- `src/core/phonics/patterns.json` — pattern database (~200 rules)
- `src/core/phonics/syllabifier.ts` — syllable splitting logic
- `src/core/phonics/hints.ts` — hint text generation
- `tests/features/phonics-engine.feature`
- `tests/steps/phonics.steps.ts`
- `tests/unit/phonics-engine.test.ts`

**Key contract:**
```typescript
// Implement these functions matching the PhonicsResult interface
function analyzeWord(word: string): PhonicsResult;
function getPatternFamily(patternId: string): string[];
function generateHint(pattern: DetectedPattern): string;
```

**No dependencies on:** React, Dexie, audio, themes, or any other workstream. Pure TypeScript + JSON.

---

#### WS-2: Spaced Repetition & Word Selection

**Time:** ~4-6 hours
**Dependencies:** WS-0 only
**Spec sections:** §5.1.6 (Spaced Repetition System), §5.1.4 (word selection algorithm)
**Inputs:** `WordStats[]`, `WordList` (active), `Word[]` (lifetime pool)
**Outputs:** `ReviewSchedule`, `SessionWordSelection`, bucket transitions
**Files to create:**
- `src/core/spaced-rep/scheduler.ts` — SM-2 adaptation, interval calculation
- `src/core/spaced-rep/buckets.ts` — bucket transition logic
- `src/core/spaced-rep/difficulty.ts` — difficulty score computation
- `src/core/word-selection/selector.ts` — session mix algorithm (60/30/10 ratios, test-day proximity boost)
- `tests/features/spaced-repetition.feature`
- `tests/features/word-selection.feature`
- `tests/steps/spaced-rep.steps.ts`
- `tests/steps/word-selection.steps.ts`
- `tests/unit/spaced-rep.test.ts`
- `tests/unit/difficulty-score.test.ts`

**Key contract:**
```typescript
function updateWordStats(stats: WordStats, result: TechniqueResult): WordStats;
function calculateNextReview(stats: WordStats): Date;
function transitionBucket(stats: WordStats): WordBucket;
function computeDifficulty(stats: WordStats): number;
...
**No dependencies on:** React, Dexie, audio, phonics, themes. Pure algorithms operating on interfaces.

---

#### WS-3: Data Layer & Profile Management

**Time:** ~6-8 hours
**Dependencies:** WS-0 only
**Spec sections:** §4.2 (Data Model), §5.1.1 (Profile Management), §5.1.3 (Word List Management)
**Inputs/Outputs:** CRUD operations on all Dexie tables
**Files to create:**
- `src/data/repositories/profile-repo.ts`
- `src/data/repositories/word-list-repo.ts`
- `src/data/repositories/word-repo.ts`
- `src/data/repositories/stats-repo.ts`
- `src/data/repositories/session-repo.ts`
- `src/data/repositories/streak-repo.ts`
- `src/data/import-export.ts` — export, import, merge (import-wins), replace
- `src/data/sync.ts` — sync queue management
- `tests/features/profile-management.feature`
- `tests/features/word-list-management.feature`
- `tests/steps/profile.steps.ts`
- `tests/steps/word-list.steps.ts`

**Key contract:**
```typescript
// Repository pattern — each entity gets a repo
interface ProfileRepository {
  create(profile: Omit<Profile, 'id'>): Promise<Profile>;
  getById(id: string): Promise<Profile | null>;
  getAll(): Promise<Profile[]>;
  update(id: string, data: Partial<Profile>): Promise<Profile>;
  delete(id: string): Promise<void>;           // cascade deletes all child data
  clearData(id: string): Promise<void>;         // reset stats/sessions, keep profile+lists
}

interface ImportExportService {
  exportProfile(profileId: string): Promise<ExportPayload>;
  importProfile(payload: ExportPayload, strategy: ImportStrategy): Promise<ImportResult>;
}
```

**Testing note:** Use `fake-indexeddb` for all tests. No real browser required.
...
**Time:** ~3-4 hours
**Dependencies:** WS-0 only
**Spec sections:** §5.1.4 (audio support — layered)
**Inputs:** `Word.text`, syllable chunks from phonics
**Outputs:** Audio playback (via `AudioProvider` interface)
**Files to create:**
- `src/audio/tts.ts` — Web Speech API provider
- `src/audio/dictionary.ts` — dictionary API provider (fetch + cache)
- `src/audio/custom.ts` — custom recorded audio provider
- `src/audio/manager.ts` — layered audio manager (tries providers in priority order)
- `tests/features/audio.feature`
- `tests/steps/audio.steps.ts`

**Key contract:**
```typescript
// Each provider implements AudioProvider from contracts
// AudioManager orchestrates them:
interface AudioManager {
  speak(word: string): Promise<void>;          // uses highest-priority available provider
  speakSlowly(word: string): Promise<void>;
  speakChunks(chunks: string[], delayMs?: number): Promise<void>;
...
**Testing note:** Mock `SpeechSynthesis` API in tests. Test provider fallback ordering.
...
**Time:** ~3-4 hours
**Dependencies:** WS-0 only
**Spec sections:** §5.1.3 (camera capture / OCR)
**Inputs:** `Blob` (camera image)
**Outputs:** `OcrResult` (extracted words + confidence)
**Files to create:**
- `src/ocr/local.ts` — Tesseract.js wrapper implementing `OcrProvider`
- `src/ocr/remote.ts` — remote server fallback implementing `OcrProvider`
- `src/ocr/manager.ts` — tries local first, falls back to remote if available
- `tests/features/ocr-import.feature`
- `tests/steps/ocr.steps.ts`

**Key contract:**
```typescript
// Both providers implement OcrProvider from contracts
// OcrManager orchestrates:
interface OcrManager {
  extractWords(image: Blob): Promise<OcrResult>;
  setRemoteEndpoint(url: string): void;  // Termux or cloud server
}
```

**Testing note:** Use fixture images of typed word lists. Test confidence thresholds and word cleaning (removing artifacts, normalizing whitespace).

---

#### WS-6: Theme Engine & Rewards

**Time:** ~4-6 hours
**Dependencies:** WS-0 only
**Spec sections:** §5.1.7 (Reward System / Theme Engine)
**Inputs:** `AppEvent` (word:attempted, session:ended), `Theme` config
**Outputs:** `RewardEvent`, theme visual config, milestone state
**Files to create:**
- `src/themes/engine.ts` — theme manager, reward calculator
- `src/themes/dragon-forge/theme.ts` — Dragon Forge theme definition
- `src/themes/monster-lab/theme.ts` — Monster Lab theme definition
- `src/themes/star-trail/theme.ts` — Star Trail theme definition
- `src/features/rewards/reward-tracker.ts` — progress tracking logic
- `tests/features/rewards.feature`
- `tests/steps/rewards.steps.ts`
...
interface ThemeEngine {
  ...
  getAllThemes(): Theme[];
  calculateReward(event: AppEvent, currentProgress: number): RewardEvent;
  getMilestoneStatus(profileId: string, themeId: string): MilestoneStatus;
}
```

**No dependencies on:** React for the logic layer. Theme assets (colors, sounds) are data, not components. The React rendering of rewards happens in WS-8.

---

#### WS-7: Accessibility System

**Time:** ~2-3 hours
**Dependencies:** WS-0 only
**Spec sections:** §5.1.2 (Visual Accessibility Settings), §7 (Visual Accessibility Deep Dive)
**Inputs:** `AccessibilitySettings` from profile
**Outputs:** CSS custom properties, React context, `useAccessibility()` hook
**Files to create:**
- `src/accessibility/settings.ts` — settings state management, CSS variable mapping
- `src/accessibility/hooks.ts` — `useAccessibility()` React hook
- `src/accessibility/presets.ts` — named presets (e.g., "Paul's settings")
- `src/accessibility/defaults.ts` — conservative defaults for double vision
...
interface AccessibilityManager {
  applySettings(settings: AccessibilitySettings): void;  // sets CSS custom properties
  getDefaults(): AccessibilitySettings;
  savePreset(name: string, settings: AccessibilitySettings): void;
  getPresets(): NamedPreset[];
}
...
#### WS-8: Integration — Practice Session, Dashboard, PWA Shell

**Time:** ~8-12 hours
**Dependencies:** WS-1 through WS-7 (all must be complete or stubbed)
**Spec sections:** §5.1.4 (Practice Session), §5.1.5 (Adaptive Session), §5.1.8 (Streak), §5.1.9 (Dashboard), §5.1.11 (PWA), §8 (Session UX Flow)
**This is where it all comes together.**

**Files to create:**
- `src/features/practice/session-controller.ts` — orchestrates word selection → phonics → audio → input → scoring → rewards
- `src/features/practice/letter-bank.tsx` — letter bank React component
- `src/features/practice/scaffolding.tsx` — phonics scaffolding display
- `src/features/practice/session-summary.tsx` — end-of-session celebration
- `src/core/adaptive/engine.ts` — engagement signal processing → adaptive actions
- `src/features/dashboard/progress-view.tsx` — streaks, readiness, lifetime health
- `src/features/dashboard/readiness-indicator.tsx` — test readiness gauge
- `src/features/dashboard/word-detail.tsx` — per-word history view
- `src/features/profiles/profile-selector.tsx` — profile selection screen
- `src/features/profiles/settings-panel.tsx` — parent PIN-protected settings
- `src/features/word-lists/list-editor.tsx` — word list CRUD UI
- `src/features/word-lists/camera-capture.tsx` — photo → OCR → editable list
- `src/features/feedback/feedback-form.tsx` — send feedback UI
- `src/features/onboarding/first-run.tsx` — first-time setup wizard
- `src/pwa/service-worker.ts` — Workbox config
- `src/pwa/manifest.ts` — PWA manifest
- `src/pwa/qr.ts` — QR code generation for sharing
- `src/App.tsx` — root component, routing, event bus wiring
- `tests/features/practice-session.feature`
- `tests/features/adaptive-session.feature`
- `tests/features/streak-tracking.feature`
- `tests/features/offline-mode.feature`
- `tests/e2e/full-session.spec.ts`
- `tests/e2e/pwa-install.spec.ts`
- `tests/e2e/offline-practice.spec.ts`

**Integration contract:** WS-8 imports from all other workstreams via their public APIs. It never reaches into internal implementation details. If a workstream isn't ready, WS-8 uses a stub that implements the same interface:

```typescript
// Example: if WS-1 (phonics) isn't ready, WS-8 uses a stub
const stubPhonics: PhonicsEngine = {
  analyzeWord: (word) => ({
    syllables: [word],
    phonemes: [],
    patterns: [],
  ...
  getPatternFamily: () => [],
  generateHint: () => "Sound it out!",
};
```

### 13.3 Dependency & Parallelism Rules

1. **WS-0 completes first.** It takes ~1-2 hours. Once done, all other workstreams begin in parallel.
2. **WS-1 through WS-7 have zero dependencies on each other.** They only import from `src/contracts/`. They can run simultaneously with full agent teams.
3. **WS-8 begins after WS-0** and develops against stubs. As WS-1–7 complete and merge, stubs are replaced with real implementations. WS-8 should be the last to merge its PR.
4. **No workstream modifies `src/contracts/types.ts`** without Coordinator approval. Interface changes require a contract amendment PR reviewed by the Coordinator.
5. **Every PR must pass `npm run typecheck && npm test`** before merge. The shared types ensure compile-time compatibility.
6. **Feature files (.feature) are the acceptance criteria.** An agent's workstream is done when all its scenarios pass.

### 13.4 Agent Assignment Summary

| Workstream | Agent Role | Spec Sections | Est. Hours | Parallel Group |
|-----------|------------|---------------|------------|----------------|
| WS-0: Foundation | Coordinator | §4.1, §12.1, §10.7 | 1-2 | — (first) |
| WS-1: Phonics Engine | Specialist | §6, §5.1.4 | 4-6 | A (parallel) |
| WS-2: Spaced Rep + Word Selection | Specialist | §5.1.6, §5.1.4 | 4-6 | A (parallel) |
| WS-3: Data Layer + Profiles | Specialist | §4.2, §5.1.1, §5.1.3 | 6-8 | A (parallel) |
| WS-4: Audio System | Specialist | §5.1.4 | 3-4 | A (parallel) |
| WS-5: OCR Pipeline | Specialist | §5.1.3 | 3-4 | A (parallel) |
| WS-6: Theme Engine + Rewards | Specialist | §5.1.7 | 4-6 | A (parallel) |
| WS-7: Accessibility | Specialist | §5.1.2, §7 | 2-3 | A (parallel) |
| WS-8: Integration + UI + PWA | Integrator | §5.1.4-5, §5.1.8-9, §5.1.11, §8 | 8-12 | B (after A) |

**Optimal team size:** 4-5 agents. Coordinator handles WS-0 then takes WS-8. Three specialist agents each take 2 workstreams from WS-1–7 sequentially. Total Phase 1 wall-clock time: ~2-3 days with a 4-agent team vs. ~6 weeks sequential.

### 13.5 Merge Order

```
1. WS-0: Foundation         → main (unblocks all)
2. WS-1 through WS-7        → main (any order, parallel PRs)
3. WS-8: Integration         → main (last, after all WS-1–7 merged)
4. E2E test suite            → main (final validation)
```
...
| Parallel architecture | Shared contracts + interface-only dependencies between modules | Enables agent team parallelism, reduces merge conflicts | Never — this is foundational |
...
### Phase 1: MVP (Parallel Agent Implementation)

**Sequential estimate:** ~6 weeks. **Parallel with 4-5 agents:** ~2-3 days.

**Step 1 — Foundation (Coordinator, ~1-2 hours):**
Scaffold project, define shared contracts (`src/contracts/types.ts`), Dexie schema, CI pipeline, test fixtures. All agents blocked until this merges.

**Step 2 — Parallel Workstreams (All agents, ~6-8 hours each):**
All workstreams begin simultaneously after WS-0 merges. Each agent implements against shared contracts, writes BDD scenarios, and opens a PR.

| Agent | First Workstream | Then |
|-------|-----------------|------|
| Coordinator | WS-3: Data Layer + Profiles (6-8h) | → WS-8: Integration |
| Agent A | WS-1: Phonics Engine (4-6h) | → WS-4: Audio System (3-4h) |
| Agent B | WS-2: Spaced Rep + Word Selection (4-6h) | → WS-5: OCR Pipeline (3-4h) |
| Agent C | WS-6: Theme Engine + Rewards (4-6h) | → WS-7: Accessibility (2-3h) |

**Step 3 — Integration (Coordinator, ~8-12 hours):**
WS-8 wires all modules together: practice session UI, dashboard, PWA shell, onboarding, feedback. Begins with stubs, swaps in real implementations as WS-1–7 PRs merge.

**Step 4 — E2E Validation (~2-3 hours):**
Full Playwright E2E suite on merged codebase. Fire tablet smoke test on real device.
